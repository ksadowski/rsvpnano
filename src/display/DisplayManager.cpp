...
// Resolve a Unicode code point to its EmbeddedSerifGlyph. ASCII (32..126) is an
// O(1) array index; everything else binary-searches the sparse extras table
// generated by tools/generate_embedded_serif_font.py. Unknown code points fall
// back to '?'.
const EmbeddedSerifGlyph &glyphFor(uint32_t cp) {
  if (cp >= kEmbeddedSerifFirstChar && cp <= kEmbeddedSerifLastChar) {
    return kEmbeddedSerifGlyphs[cp - kEmbeddedSerifFirstChar];
  }
  ...
  if (kEmbeddedSerifExtraGlyphCount > 0) {
    ...
    while (lo < hi) {
      ...
      if (midCp == cp) {
        const EmbeddedSerifExtraGlyph &extra = kEmbeddedSerifExtraGlyphs[mid];
        // Reuse the matching ASCII fallback layout by returning a reference to
        // a static glyph populated from the extra. Constructing a fresh
        // EmbeddedSerifGlyph each call avoids storing a parallel table.
        static EmbeddedSerifGlyph cached;
  ...
  return kEmbeddedSerifGlyphs[static_cast<uint8_t>('?') - kEmbeddedSerifFirstChar];
}

const EmbeddedSerif70Glyph &glyph70For(uint32_t cp) {
  if (cp >= kEmbeddedSerif70FirstChar && cp <= kEmbeddedSerif70LastChar) {
    return kEmbeddedSerif70Glyphs[cp - kEmbeddedSerif70FirstChar];
  }
  ...
  if (kEmbeddedSerif70ExtraGlyphCount > 0) {
    ...
    while (lo < hi) {
      ...
      if (midCp == cp) {
        const EmbeddedSerif70ExtraGlyph &extra = kEmbeddedSerif70ExtraGlyphs[mid];
        static EmbeddedSerif70Glyph cached;
        cached.bitmapOffset = extra.bitmapOffset;
        cached.xOffset = extra.xOffset;
        cached.width = extra.width;
        cached.xAdvance = extra.xAdvance;
  ...
  return kEmbeddedSerif70Glyphs[static_cast<uint8_t>('?') - kEmbeddedSerif70FirstChar];
}
...
TextLayoutMetrics serifWordLayout(const String &word, int focusIndex, int divisor = 1) {
  ...
  for (size_t i = 0; i < cps.size(); ++i) {
    const EmbeddedSerifGlyph &glyph = glyphFor(cps[i]);
    ...
    if (i + 1 < cps.size()) {
      const EmbeddedSerifGlyph &nextGlyph = glyphFor(cps[i + 1]);
...
TextLayoutMetrics serif70WordLayout(const String &word, int focusIndex) {
  ...
  for (size_t i = 0; i < cps.size(); ++i) {
    const EmbeddedSerif70Glyph &glyph = glyph70For(cps[i]);
    ...
    if (i + 1 < cps.size()) {
      const EmbeddedSerif70Glyph &nextGlyph = glyph70For(cps[i + 1]);
...
void DisplayManager::drawGlyph(int x, int y, uint32_t cp, uint16_t color) {
  const EmbeddedSerifGlyph &glyph = glyphFor(cp);
  if (glyph.width == 0) {
    return;
  }
...
void DisplayManager::drawSerifGlyphScaled(int x, int y, uint32_t cp, uint16_t color, int divisor) {
  divisor = std::max(1, divisor);
  const EmbeddedSerifGlyph &glyph = glyphFor(cp);
  if (glyph.width == 0) {
    return;
...
void DisplayManager::drawSerif70Glyph(int x, int y, uint32_t cp, uint16_t color) {
  const EmbeddedSerif70Glyph &glyph = glyph70For(cp);
  if (glyph.width == 0) {
    return;
  }
...
void DisplayManager::drawSerifGlyphScaledPercent(int x, int y, uint32_t cp, uint16_t color,
                                                 uint8_t scalePercent) {
  if (scalePercent >= 100) {
...
void DisplayManager::drawSerifTextAt(const String &text, int x, int y, uint16_t color,
  ...
  for (size_t i = 0; i < cps.size(); ++i) {
    ...
    const int xOffset = scaledSignedAdvance(static_cast<int>(glyph.xOffset), divisor);
    const int width = glyph.width == 0 ? 0 : scaledAdvance(static_cast<int>(glyph.width), divisor);
    drawSerifGlyphScaled(cursorX + xOffset, y, cps[i], color, divisor);
    int tracked = trackedAdvanceScaled(static_cast<int>(glyph.xAdvance), divisor, i, cps.size());
    if (i + 1 < cps.size()) {
      const EmbeddedSerifGlyph &nextGlyph = glyphFor(cps[i + 1]);
      tracked -= opticalKerningAdjustment(
...
void DisplayManager::drawSerif70TextAt(const String &text, int x, int y, uint16_t color) {
  ...
  for (size_t i = 0; i < cps.size(); ++i) {
    const EmbeddedSerif70Glyph &glyph = glyph70For(cps[i]);
    drawSerif70Glyph(cursorX + static_cast<int>(glyph.xOffset), y, cps[i], color);
...
void DisplayManager::drawSerifTextScaledAt(const String &text, int x, int y, uint16_t color,
  ...
  for (size_t i = 0; i < cps.size(); ++i) {
    ...
    drawSerifGlyphScaledPercent(cursorX + xOffset, y, cps[i], color, scalePercent);
    int tracked =
        trackedAdvanceScaledPercent(static_cast<int>(glyph.xAdvance), scalePercent, i, cps.size());
    if (i + 1 < cps.size()) {
      const EmbeddedSerifGlyph &nextGlyph = glyphFor(cps[i + 1]);
